/*
obs-websocket (modified to work with )
...
typedef struct obs_websocket_request_response {
...
typedef struct obs_websocket_request_callback {
	obs_websocket_request_callback_function callback;
	void *priv_data;
} obs_websocket_request_callback;
...
static inline obs_websocket_request_response *obs_websocket_call_request(const char *request_type, obs_data_t *request_data)
{
	if (!obs_websocket_ensure_ph())
		return NULL;
	...
	obs_websocket_request_response* ret = (obs_websocket_request_response *)calldata_ptr(&cd, "response");
							 ...
							 obs_websocket_request_callback_function request_callback, void *priv_data)
{
	...
	obs_websocket_request_callback cb = {0};
